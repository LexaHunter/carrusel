...
let search_cont = $("#search_input"),
    search = $("#search_bar");

const url = "http://localhost:3001/items"; // !!! Mirar filtrado desde URL.
...
search.on("keyup", function(event) {
    ...
    }).then( /* !!!!! El método $.ajax() devuelve un objeto de tipo jqXHR, que implementa la interfaz de Promise, por lo que tiene todos los métodos de 
                un objeto de tipo promesa, entre ellos .then() => Si uso el método .then() pasándole 2 funciones = resolve y 
                reject, [por lo que he visto] no hay que definir las porpiedades success y error ene l método $.ajax() = [por 
                lo que veo] asume directemente como acciones las funciones definidasn en el métddo .then() */
        function(items) { // onFulfilled = resolve
            if ($(".select_items")) {
                $(".select_items").remove();
            }
            let contItems = $("<div></div>").attr({class: "select_items"}).css({"z-index": 1}),
                option;
